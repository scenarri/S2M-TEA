...
class DIFGSM(Attack):
    ...
    def __init__(self, model, eps=8/255, alpha=2/255, steps=10, decay=0.0,
                 resize_rate=0.9, diversity_prob=0.5, random_start=False, M=1, di=True, sva=False):
    ...
    def SVA(self, x, data_size=64, kernel_size=3):
        median = kornia.filters.median_blur(x, (kernel_size, kernel_size))  # 11 in paper
        noise = stats.truncexpon(b=1.5, loc=0, scale=1).rvs([data_size, data_size])
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            grad = torch.zeros_like(images).detach().to(self.device)
