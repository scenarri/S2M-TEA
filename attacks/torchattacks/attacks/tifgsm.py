...
class TIFGSM(Attack):
    ...
    def __init__(self, model, eps=8/255, alpha=2/255, steps=10, decay=0.0, kernel_name='gaussian',
                 len_kernel=15, nsig=3, resize_rate=0.9, diversity_prob=0., random_start=False, m=5, sva=False):
    ...
    def forward(self, images, labels):
        ...
        ori_out = self.get_logits(images)
        ...
        for _ in range(self.steps):
            ...
            if self.sva:
                outputs = self.get_logits(self.SVA(adv_images))
            else:
            ...
            # depth wise conv2d
            grad = F.conv2d(grad, stacked_kernel, stride=1, padding='same', groups=1)
    ...
    def SVA(self, x, data_size=64, kernel_size=7):
        median = kornia.filters.median_blur(x, (kernel_size, kernel_size))  # 11 in original paper for 224*224 input, stats.expon() as standard exponential dist. and stats.truncexpon(b=1.5, loc=0, scale=1) for truncated one.
        noise = stats.expon().rvs([data_size, data_size])
        output = median * torch.from_numpy(noise).cuda().float()
